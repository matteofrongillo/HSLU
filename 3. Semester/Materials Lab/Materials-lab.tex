...
\begin{table*}[h!]
  ...
  \begin{tabular}{|>{\bfseries}c|p{5.5cm}|}
    ...
    \hline
    Ceramics & 
    1) High temperature resistance (High E, Low $\alpha$) \newline
...
\pph{Final microstructure at Room Temperature}
...
\figbox{$\dm \mathrm{HV} = 0.102 \cdot F \cdot \frac{2\cos(22^\circ)}{\left(\dfrac{d_1 + d_2}{2}\right)^2}$}
...
\newpage
\part{Measurement of chemical compositions}
\section{Selected methods}
\subsection{X-ray Fluorescence (XRF)}

\subsection{Optical Emission Spectrometry (OES)}

\subsection{Energy-dispersive spectroscopy (EDS, EDX, EDAX)}
\subsubsection{Line Scan EDS}

\subsubsection{EDS Area Scan}
