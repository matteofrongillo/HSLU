...
The order of the monomials is not important, but it is preferible to write
the highest degree monomial in decreasing order.
\figbox{$p(x)=ax^2-bx+c$}

\subsubsection{Sum}
We have to sum all the same degree monomials.
...
We have to mulitply factors each other with the distributive property. 
...
\section{Equations}
An equation is a formula given by the equality of the expressions.
...
\begin{itemize}
    ...
    \item $|a|=-3$ it is alway positive or zero;
...
real number that, when they replace a variable inside the equation are
giving an identity.
...
$a,b \in \mathbb{R},\ x+a=b$, let's isolate the variable $x-a-a=b-a \Rightarrow x=b-a$

\subsubsection{Tool 2}
$a,b \in \mathbb{R},\ ax=b$, let's isolate the variable $\frac{ax}{a}=\frac{b}{a} \Rightarrow x=\frac{b}{a}$
