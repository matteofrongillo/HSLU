...
\subsection{Conversion table of degrees and radiants}
\begin{center}
...
\newpage
\subsection{Trigonometric funcions in the unit circle}
\begin{center}
...
Because we have a 90$^\circ$ angle, we can use Pitagora: 
\figbox{$\overline{OH}^{\,2}+\overline{PH}^{\,2}=\overline{OP}^{\,2}$}

Then, we can compute that:
\figbox{$\sin(\theta)^2 + \cos(\theta)^2 = 1 \qquad \forall\, \theta \in \mathbb{R}$}

\subsubsection{Example with 45$^{\circ}$}
When $\theta=45^{\circ}$, then $\sin(\theta)=\cos(\theta) \Rightarrow 2\cos(\theta)=1
...
or 180$^{\circ}$, that is when we have a vertical line.
