...
\subsection{\color{red}Precision vs. Accuracy}
...
\section{Theory of Sampling (TOS)}
\begin{enumerate}
...
\newpage
\subsection{\color{red}TOS' basic terms}
...
\pph{Analyte}
An analyte is the specific substance or chemical component in a sample that is being identified or measured.

\section{General Principles (GPs)}
\subsection{FSP: Fundamental Sampling Principle}
\begin{itemize}
...
\subsection{LDT: Lot Dimensionality Transformation}
\begin{minipage}{0.5\textwidth}
    \begin{itemize}
...
\newpage
\subsection{PSC: Principle of Sampling Correctness (bias-free sampling)}
...
\begin{itemize}
    ...
    \item TAE = Total Analytical Error (e.g. $\pm 5\%$)
\end{itemize}
...
\subsubsection{Correct sampling errors}
\begin{itemize}
    \item Correct sampling errors cause random effects (imprecision) due to material heterogeneity effects
    \item Can never be completely eliminated but should be reduced as far as possible
\end{itemize}

Total material heterogeneity in a lot must be distinguished as two components:
\begin{enumerate}
    \item \textbf{Constitutional (compositional) heterogeneity (CH)}
    \item \textbf{Distributional heterogeneity (DH)}:
    \begin{itemize}
        \item DH$_\text{L}$ reflects the irregular spatial distribution of the analyte
        \item DH$_\text{L}$ is caused by the inherent tendency of particles to cluster and segregate locally (grouping) and more pervasively throughout the lot (segregation)
        \item In TOS, this is viewed as reflecting the Grouping and Segregation Error (GSE)
        \item DH$_\text{L}$ can be counteracted by the process of mixing (SUO 2) and/or by suitably-deployed, problem- dependent composite sampling (SUO 1) with a sampling tool allowing a high number of increments
    \end{itemize}
\end{enumerate}

\newpage
\subsubsection{Sampling Unit Operations (SUOs)}
\paragraph{SUO 1: Composite Sampling}
\begin{itemize}
    \item Composite Sampling reduces Grouping and Segregation effects
    \item Heterogeneity is many times not observable with the naked eye
    \item Composite samples need often mass reduction $\to$\ representative mass reduction (SUO 4)
\end{itemize}
\begin{center}
    \includegraphics[width=.7\textwidth]{media/SUO1.png}
\end{center}

\pph{SUO 2: Mixing / Blending}
Mixing is trivial but important process (decreases segregation effects - GSE) - still NO excuse for grab sampling

Mixing reduces contributions to sampling variation from the Grouping and Segregation Error (GSE):
\[s^2(\mathrm{(GSE)} = \zeta \cdot \gamma \cdot s^2\mathrm{(FSE)})\]

where $\zeta$ is the Segregation factor and $\gamma$ is the Grouping
factor (unaffected by mixing - reduced only by selecting smaller increments)

\begin{center}
    \includegraphics[width=\textwidth]{media/SUO2.png}
\end{center}

\paragraph{Constitutional heterogeneity (CH)}
\begin{itemize}
    \item CH describes the heterogeneity depending on the chemical and/ or physical differences between individual ``constituent units'' in the lot (e.g., particles, grains, or kernels)
    \item When a lot (L) is sampled, CH$_\text{L}$ manifests itself in the form of the Fundamental Sampling Error (FSE) effect
    \item CH$_\text{L}$ increases when the compositional difference between fragments increases; CH$_\text{L}$ can only be reduced by comminution/crushing (SUO 3)
\end{itemize}

\pph{SUO 3: Comminution}
FSE can be reduced (never eliminated) by particle size reduction (crushing)

\pph{SUO 4: Mass Reduction (sub-sampling)}
Representative sampling is always a multistage process $\to$\
Mass reduction is as important as primary, secondary [...] sampling

Mass reduction method / device can have influence on:\\
Estimating true concentration (accuracy), Sampling variation (precision / reproducibility), Time consumption, Ease of cleaning, Loss of material, User dependency, Cost

\subsection{SSI: Sampling Scale Invariance}
Theory of Sampling is scale invariant; all principles need to be followed
independent of lot and increment size.

\subsection{PSS: Sampling Simplicity (primary sampling + mass reduction)}
\begin{enumerate}
    \item Sampling is never a one-shot operation
    \item Sampling is always a multi-stage process: Primary sampling + Representative Mass-reduction
\end{enumerate}

\subsection{LHC: Lot Heterogeneity Characterization}
\begin{itemize}
    \item An appropriate sampling procedure can only be designed knowing the lot heterogeneity quantitatively
    \item For 3-dimensional/stationary lots: A replication experiment will reveal Total Sampling Error as well as sampling steps generating the largest variance
    \item For 1-dimensional lots: Complete empirical sampling error estimation - variographic analysis
\end{itemize}

\subsubsection{Variographic Analysis (Variography)}
\begin{itemize}
    \item 1-dimensional heterogeneity characterization
    \item 1-D lots: processes, long stationary piles, ordered series etc.
    \item Valuable information about lot variation (trends, periodic phenomena, shifts)
\end{itemize}

\begin{center}
    \includegraphics[width=.8\textwidth]{media/variography.png}
\end{center}

“Lag” is the distance between increments (samples) along the time or
spatial dimension

A variogram displays the total variation as a function of the lag

\newpage
\pph{Variography procedure}
\textbf{Lag 1}:\\
Increment 1 is compared with Increment 2, Increment 2 is compared with
Increment 3, ..., Increment $i$ is compared with Increment $i+1$

\textbf{Lag 2}:\\
Increment 1 is compared with Increment 3, Increment 2 is compared with
Increment 4, ..., Increment $i$ is compared with Increment $i+2$

\textbf{Lag 3}:\\
Increment 1 is compared with Increment 4, Increment 2 is compared with
Increment 5, ..., Increment $i$ is compared with Increment $i+3$

\textbf{Lag $\mathbf{n}$}:\\
Increment 1 is compared with Increment $1+n$, Increment 2 is compared with
Increment $2+n$, ..., Increment $i$ is compared with Increment $i+n$

\part{9}
\section{Multi-criteria decision analysis (MCDA/MCA)}
Complex decisions require various inputs:
\begin{itemize}
    \item Data (measurements, observations, experiences etc.)
    \item Risk analysis (what effects might a certain decision have?)
    \item Cost analysis (or CBA)
    \item Preferences of certain stakeholders
\end{itemize}

MCDA is a mathematical methodology incorporating values of decision makers to select best
solution for a certain problem $\to$\ Scientifically defensible decision
\begin{itemize}
    \item Multi-criteria analysis identifies criteria for choosing the best alternative among competing projects or products
    \item Criteria may include air/water pollution, GHG emissions, EROI, economic value, social criteria, etc.
    \item All criteria are evaluated on common basis allowing comparison between criteria and alternatives $\to$\ overall multi-criteria score
\end{itemize}

\paragraph{\color{red}Steps for MCDA}
\begin{itemize}
    \item Define aim and scope of analysis
    \item Selection of decision criteria and weight scale
    \item Calculation of scores for each criterion
    \item If weight scale is modified $\to$\ recalculation of criteria weights
    \item Interpretation of results and sensitivity analysis
\end{itemize}

\subsection{MCDA structure}
\begin{enumerate}
    \item Problem identification $\to$\ Defined in terms of overall structure and relevant stakeholders
    \item Problem structuring:
    \begin{itemize}
        \item Definition of alternatives and criteria
        \item Criteria are set of properties that describe alternative performance (e.g. cost, efficiency, environmental impact)
    \end{itemize}
    \item Assessment:
    \begin{itemize}
        \item Alternatives and criteria are given numerical values (e.g. LCA, cost assessment, risk assessment, long-term impact)
        \item Alternatives are scored against the criteria
        \item Criteria can also be weight according to value or importance
    \end{itemize}
    \item Initial results and sensitivity analysis
    \item Decision/planning $\to$\ MCDA output is used to make decision or further planning
\end{enumerate}

\subsubsection{Problem identification}
Focussing questions:
\begin{itemize}
    \item What is the significance of the problem?
    \item Can the problem be broken down into key components (criteria)?
    \item Does the problem involve several alternatives that need to be compared?
    \item Are alternatives already known or is further research required?
    \item What is the pre-conceived judgement of the problem?
\end{itemize}

\subsubsection{Problem structuring}
\begin{itemize}
    \item Identifying (decision) criteria $\to$\ Purpose is to develop hierarchical ``value tree''
    \item Pre-emptive criteria are used to screen out alternatives before the comparison:
    \begin{itemize}[label=$\to$]
        \item Only alternatives in a certain cost range are considered
        \item Pre-emptive criteria are not shown in value tree
    \end{itemize}
    \item Level of detail within tree hierarchy should be balanced
    \item Avoid criteria that measure the same attribute
\end{itemize}

\begin{center}
    \includegraphics[width=\textwidth]{media/mcda.png}
\end{center}

\paragraph{Focusing questions}
\begin{itemize}
    \item Which are the alternatives?
    \item Do unknown alternatives exist?
    \item Do certain alternatives overlap $\to$\ can they be grouped?
    \item Which criteria exist to evaluate the differences between certain alternatives?
    \item Are these criteria pre-emptive (if yes, which alternatives can be screened out?)
    \item What is the logical hierarchical structure of the criteria (e.g. purchase price, maintenance)
\end{itemize}

\newpage
\subsubsection{Assessment}
Assignment of numerical values to criteria and alternatives.

Decision on applied scale (e.g. ordinal, interval, ratio):
\begin{enumerate}
    \item Ordinal is the measurement of consumers’ satisfaction:
    \begin{itemize}
        \item Very dissatisfied, somewhat dissatisfied, somewhat satisfied, very satisfied
        \item Allows comparison but does not capture difference between two levels
    \end{itemize}
    \item Interval is numerical scales in which intervals have the same interpretation:
    \begin{itemize}
        \item E.g. Celsius scale of temperature (difference between 10 and 15 degrees represents same temperature difference as between 0 and 5)
        \item 5-degree interval has same physical meaning (kinetic energy of molecules)
    \end{itemize}
    \item Ratio is the most informative scale:
    \begin{itemize}
        \item Interval scale with zero position (absence of quantity being measured)
        \item E.g. amount of money in your wallet
    \end{itemize}
    \item Selection of scale depends on data characteristics (quantitative, qualitative)
    \item Linear vs non-linear scale:
    \begin{itemize}
        \item E.g. an ideal house size does not need to be the largest/smallest option
        \item Most preferred value could occur in the middle range of measured values
    \end{itemize}
\end{enumerate}

\paragraph{Focussing questions}
\begin{itemize}
    \item What data is available?
    \item Quality of available data?
    \item Do some criteria require qualitative scoring?
    \item Do certain criteria require non-linear scales?
    \item How can numerical values (with different units) be translated into a score (no unit)
    \item Can each criteria be scored on its own without considering other criteria? If not criteria/value tree should be redefined
\end{itemize}

\subsection{Multi-criteria analysis}
The weighing of the criteria have an important impact on the outcome of the analysis
$\to$\ (Environmental) weight sets have been developed in various research studies
...
\subsection{Framework vs Tool}
\pph{Framework}
A framework is a structured foundation that provides rules,
components, and a predefined way to build something.

\pph{Tool}
A tool is a specific instrument or software used to perform a
particular task.

\subsection{What is GIS}
\subsubsection{Definition}
A geographic information system (GIS) is a framework for gathering, managing, and analyzing data. It
analyzes spatial location and organizes layers of information into visualizations using maps.

\textbf{G}eographic: relates to the surface of the earth

\textbf{I}nformation: is knowledge derived from study, experience, or instruction

\textbf{S}ystem: is a group of interacting, interrelated or
interdependent elements forming a complex whole

\subsubsection{Spatial data}
Information about locations and shapes of geographic features and the
relationships between them.

\subsubsection{Applications}
\begin{minipage}[t]{0.48\textwidth}
    \textbf{Natural environment:}
    \begin{itemize}
        \item Forestry
        \item Modelling of habitats
        \item Habitat protection
        \item Environmental impact assessment
        \item Landscape protection
        \item Management of natural resources
        \item Modelling of emissions
        \item Modelling of natural hazards
        \item etc.
    \end{itemize}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
    \textbf{Anthropogenic environment:}
    \begin{itemize}
        \item Local and regional planning
        \item Selection of company locations
        \item Population statistics
        \item Infrastructure management
        \item Landscape protection
        \item Planning of electricity grids
        \item Route selection and fleet management
        \item Vehicle navigation
        \item etc.
    \end{itemize}
\end{minipage}

\subsubsection{GIS tasks and questions}
\begin{center}
    \renewcommand{\arraystretch}{1.8}
    \begin{tabularx}{\textwidth}{|l|X|}
        \hline
        \textbf{Task} & \textbf{Question}\\
        \hline Localisation/Orientation & What can be found at this location (e.g. shops, restaurants etc.)?\\
        \hline \multirow{2}{*}{Thematic query} & Which agricultural areas are used for vegetable cultivation? (and where are they situated?)\\
        \cline{2-2}
        & Which municipalities have more than 10'000 inhabitants? (and where are they situated?)\\
        \hline Temporal query & How has forest area changed since 1900?\\
        \hline Spatial query & How many people in Wales live 300 meters away from a mobile communications antenna?\\
        \hline Network analysis & What is the fastest way from Zurich to Geneva?\\
        \hline Simulation of processes & How would a forest fire spread or which path would a lava stream follow?\\
        \hline Decision support & Which areas would be affected by the bursting of a dam and where would be safe evacuation routes?\\
        \hline
    \end{tabularx}
\end{center}

\subsection{GIS components}
\subsubsection{Data}
\begin{itemize}
    \item Data provides information used within a GIS
    \item A GIS often incorporates data from multiple sources:
    \begin{itemize}
        \item Accuracy defines the quality of the GIS
        \item GIS quality determines the level of detail of questions/problems that may be asked/answered
    \end{itemize}
    \item Data sources might be: Remote sensing, Paper maps, Statistical reports, Databases, ...
\end{itemize}

\subsubsection{Remote sensing (RS)}
\begin{itemize}
    \item Obtaining information in one place without physically being there
    \item RS methods: airplane, satellite, drone etc.
    \item RS cameras can see things beyond human vision (between \tilde 400 - 700 nm) $\to$\ From short wavelengths (like X-rays) to long wavelengths (like radio waves)
    \item Each object has its own unique spectral signature, depending on its chemical composition
\end{itemize}

\pph{Active RS}
Active instruments provide their own energy (electromagnetic radiation)
to illuminate the object or scene they observe

\pph{Passive RS}
Passive sensors detect natural energy (sunlight) that is reflected or
emitted from the observed scene

\subsubsection{Software}
\begin{itemize}
    \item Standard software includes packages used for databases, drawings, statistics and imaging
    \item Software used must match the needs and skills of the end user
    \item Most popular GIS software programs are ArcGIS (commercial) and QGIS (open source)
\end{itemize}

\subsubsection{Hardware}
Type of hardware determines speed at which a GIS will operate (may influence type of software
used)

\subsubsection{Method}
\includegraphics[width=\textwidth]{media/methods.png}

\subsubsection{Picture methods}
\includegraphics[width=\textwidth]{media/GIS_pictures.png}
Each feature (point, line, polygon or raster) within a GIS layer is
represented as an entry in a data table

\pph{Raster picture}
A geographic feature (e.g. land cover) is represented as single square cells

\pph{Vectorial picture}
Features are represented as points, lines, polygons

\subsubsection{Raster and vector models}
\begin{center}
    \renewcommand{\arraystretch}{1.35}
    \renewcommand{\tabularxcolumn}[1]{m{#1}}
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}m{1cm}|X|X|}
        \cline{2-3}
        \multicolumn{1}{c|}{} & \centering\arraybackslash\textbf{Raster model} & \centering\arraybackslash\textbf{Vector model}\\
        \hline
        \rotatebox{90}{\textbf{Advantages}} &
        \begin{itemize}[leftmargin=*, topsep=2pt, itemsep=.2em, parsep=0pt]
            \item Remote sensing imagery is typically obtained in raster format $\to$ simple data structure: a grid with a code in each cell
            \item Quantitative analysis is more intuitive
        \end{itemize}
        &
        \begin{itemize}[leftmargin=*, topsep=2pt, itemsep=.2em, parsep=0pt]
            \item Graphic output is generally more aesthetically pleasing
            \item Higher geographic accuracy because data is not dependent on grid size
            \item Network analysis and proximity operations use vector data structures
        \end{itemize}\\
        \hline
        \rotatebox{90}{\textbf{Disadvantages}} &
        \begin{itemize}[leftmargin=*, topsep=2pt, itemsep=.2em, parsep=0pt]
            \item Pixelated look, linear features and paths are difficult to display
            \item Raster datasets can become very large $\to$ they record values for each cell in an image
        \end{itemize}
        &
        \begin{itemize}[leftmargin=*, topsep=2pt, itemsep=.2em, parsep=0pt]
            \item Usually substantial interpolation is required for these data layers
        \end{itemize}\\
        \hline
    \end{tabularx}
\end{center}
...
\part{12}
\section{Non-renewable energy sources}
\begin{itemize}
    \item Non-renewable energy resources cannot be replaced once they have been used
    \item Will not be restored (at least not for millions of years)
    \item Non-renewable energy resources include fossil fuels and nuclear fuels
\end{itemize}

\section{Characteristics of fossil fuels}
...
Any naturally occurring carbon or hydrocarbon fuel that is formed by prehistoric decomposition of
organisms (millions of years): coal, natural gas and petroleum.
Petroleum refers both to unprocessed crude oil and refined petroleum products

\subsubsection{Nuclear fuels}
Nuclear fuels release energy by nuclear fission or (nuclear fusion).
Most common nuclear fuels (radioactive metals) are uranium and plutonium.

\subsection{Usage}
In contrast to natural gas and petroleum, coal cannot be combusted directly in internal
combustion engines.
Coal is not used in transportation, but in stationary applications (e.g. electricity generation).
Coal is also used in small-scale domestic applications (e.g. cooking, space heating)

Gas and oil can be combusted directly:
Both are used for transportation and stationary applications.
Gas requires modest amount of purification.
Crude oil is refined into a range of end products
(e.g. petrol, kerosene, lubricating oil, heavy oil).

\subsection{Energy density}
\begin{itemize}
    \item Energy density = amount of energy per unit (mass or volume) available in a resource
    \item Using fuels with low energy density requires greater amount of fuel to achieve certain energy output
    \item Energy density of coal is measured in GJ/tonne (or million BTU/tonne)
    \item Energy density of oil is measured in GJ/barrel (or million BTU/barrel)
    \item Energy density of natural gas is measured in MJ/m$^3$ (or BTU/ft$^3$) at atmospheric pressure
\end{itemize}

\subsection{Estimated reserves and resources}
\subsubsection{\color{red}Reserve}
Proven quantity of a fossil fuel that is known to exist in a given location



\subsubsection{\color{red}Resource}
Estimated quantity of fuel that has yet to be fully explored and evaluated.

For each fuel an additional quantity of resource is available
(depending also on available extraction technology).

\subsection{Reserve exhaustion}
\begin{itemize}
    \item Linear model of resource exhaustion is not realistic
    \item Availability/exhaustion of reserves are depending on reserve-to-consumption ratio
    \item Individual oil and gas fields and coal mines typically follow a life cycle $\to$\ Productivity grows first, peaks and then begins to decline as wells/mines become less productive
    \item Worldwide production of a fossil fuel follows same pathway
\end{itemize}

\begin{minipage}{0.5\textwidth}
    \begin{center}
        Linear reserve:\\[1ex]
        \includegraphics[width=.9\linewidth]{media/reserve.png}
    \end{center}
\end{minipage}
\begin{minipage}{0.5\textwidth}
    \begin{center}
        Hubbert curve:\\[1ex]
        \includegraphics[width=.9\textwidth]{media/hubbert.png}
    \end{center}
\end{minipage}

\section{Fossil fuels - coal}
\subsection{Formation and extraction of coal}
\begin{itemize}
    \item Coal consists mainly of carbon, but contains elements such as hydrogen, oxygen, nitrogen, sulphur as well numerous minerals
    \item Coal was mainly formed during the Carboniferous period:
    \begin{itemize}
        \item Large amounts of organic materials grew in swamp like conditions
        \item Dead organic matter drifted to bottom of swamps and was slowly decomposed and turned into peat
        \item Over millions of years peat was compressed and turned into coal (by pressure and heat) $\to$\ The higher heat and compression level, the higher the level of carbon (‘coalification’)
    \end{itemize}
    \item Formation of coal is an on-going process
\end{itemize}

\subsection{Ranking of coal}
Coal can be broken down into four main types, depending on moisture, heating value and
carbon content.
\includegraphics[width=\textwidth]{media/coal-type.png}

\textbf{Lignite}\\
Contains 25\% - 35\% carbon and has the lowest coal heating value

\textbf{Sub-bituminous coal}\\
Contains 35\% - 45\% carbon and has slightly higher heating value than lignite coal

\textbf{Bituminous coal}\\
Contains 45\% - 86\% carbon and has the highest heating value

\textbf{Anthracite (black/hard coal)}\\
Highest concentration of carbon (between 85\% - 97\% carbon),
slightly lower heating rate than bituminous coal

\subsection{Extraction techniques}
\subsubsection{Surface mining}
\begin{itemize}
    \item Surface mining consists of mining no deeper than 200 feet (approx. 60m)
    \item Surface mining is much cheaper than underground mining
    \item Large machinery is required to remove topsoil and large layers of rock to expose coal beds
    \item Once coal has been mined normally rock and topsoil is moved back
    \item Type of surface mining depends on two factors: resource and local topography
\end{itemize}

\subsubsection{Open-pit mining}
\begin{itemize}
    \item Mainly used for extracting metal ores (e.g. gold, iron) and stone (e.g. marble, limestone)
    \item Machines dig holes and remove resource
\end{itemize}

\subsubsection{Strip mining}
\begin{itemize}
    \item Used for mineral deposits that lie close to the earth’s surface in large horizontal beds
    \item Gigantic earthmovers strip away the overburden and remove mineral deposit
\end{itemize}

\subsubsection{Contour strip mining}
\begin{itemize}
    \item Mostly used for coal (located in different coal layers)
    \item Huge shovels cut terraces into the side of a hill
    \item Earthmover removes overburden to extract the coal
\end{itemize}

\subsubsection{Underground mining (subsurface mining)}
\begin{itemize}
    \item More expensive and dangerous option
    \item Allows coal extraction from more than 1,000 feet (300m)
    \item Two large shafts are drilled into the ground:
    \begin{enumerate}
        \item First allows transport of miners and machinery to the coal
        \item Second shaft is used to transport mined coal to surface
    \end{enumerate}
\end{itemize}

\subsection{Trasportation of extracted coal}
After mining, coal is crushed into manageable pieces.

Transport methods for coal: railway, truck, conveyors, and by cargo ship

\section{Environmental impacts of mining}
\subsection{Surface mining}
\begin{itemize}
    \item Disturbed land, which is susceptible to chemical weathering and erosion by water and wind and leads to slow regrowth of vegetation
    \item Major air and water pollution:
    \begin{itemize}
        \item Wind and water erosion can spread mining waste deposit
        \item Acid mine drainage - rainwater seeping through a mine deposit to nearby streams and groundwater
        \item Emission of toxic chemicals into atmosphere
    \end{itemize}
    \item Noise and visual pollution
    \item Surface mining sites can be cleaned up and restored $\to$\ very costly
\end{itemize}

\subsection{Subsurface mining}
\begin{itemize}
    \item Disturbs less than one-tenth as much land as surface mining
    \item Less waste material is produced
    \item Lower recovery rate (more resources are left in the ground)
    \item More expensive and dangerous than surface mining
    \item Potential hazards: explosions, fires, cave-ins, diseases (black lung), subsidence (collapse of land above some underground mines)
\end{itemize}

\section{Consumption of coal}
Coal is primarily used for: Electricity production, Steel/cement production, General industry (e.g. chemicals, paper)

\subsection{Coal price}
Coal is the cheapest fossil fuel and price (USD/ton) is relatively stable

\section{Measures for minimizing air pollution}
\subsection{Preventive measures}
\begin{itemize}
    \item Selection of suitable coal fuel type
    \item Most important coal quality characteristics are: carbon content, moisture content, suphur content, nitrogen content
\end{itemize}

\subsection{Control measures}
\begin{itemize}
    \item Modern power plants are able (depending on coal type) to remove:
    \begin{itemize}
        \item $>80\%$ of NO$_\text{x}$
        \item $> 90-95\%$ of SO$_2$
        \item \tilde 99\% of incombustible parts (fly ash)
    \end{itemize}
\end{itemize}

\subsubsection{Removal of fly ash: Electrostatic precipitator}
Flue gas is forced though two electrodes:
\begin{itemize}
    \item First electrode is charged negatively
    \item Second electrode charged to a high positive voltage $\to$\ Negatively charged particles stick to collector plates
    \item Collecting plates are shaken or brushed in regular time intervals (rapping)
\end{itemize}

\begin{center}
    \includegraphics[width=.5\textwidth]{media/electrostatic.png}
\end{center}

\subsubsection{Removal of SO$_2$: Flue gas desulphurization (FGD)}
Different technologies exists to remove SO$_2$ from the flue gas:
\begin{itemize}
    \item Wet scrubber systems (most used FGD technology)
    \item Semi-dry scrubber systems
    \item Dry systems
\end{itemize}

\paragraph{Wet scrubbing}
\begin{itemize}
    \item Flue gas is ducted to a spray tower
    \item Aqueous sorbent (slurry between water and sorbent) is injected into the flue gas
    \item Typical sorbent materials are limestone (calcium carbonate) or lime (calcium oxide)
    \item Creates gypsum (used in construction and cement industry)
\end{itemize}
\begin{center}
    \includegraphics[width=.5\textwidth]{media/wetscrubbing.png}
\end{center}

\subsubsection{Removal of NO$_\text{x}$ - DeNO$_\text{x}$ process}
\begin{itemize}
    \item NO$_\text{x}$ is formed during combustion process when nitrogen reacts with oxygen in high temperatures
    \item NO$_\text{x}$ level in flue gas is reduced by injecting ammonia (NH$_3$)
    \item Reaction creates nitrogen (natural component of atmosphere) and water vapour
\end{itemize}
\begin{center}
    \includegraphics[width=.7\textwidth]{media/NOx.png}
\end{center}

\subsubsection{Removal of CO$_2$ - Carbon capture and storage (CCS)}
\begin{itemize}
    \item Removing CO$_2$ from smokestacks of coal power plants and industrial plants
    \item CO$_2$ gas is pumped into underground coal beds or abandoned oil and gas fields
    \item CO$_2$ gas can also be liquefied and injected into thick sediments under the sea floor
    \item CO$_2$ is pumped to fields and tree plantations for fertilization
\end{itemize}
\begin{center}
    \includegraphics[width=.45\textwidth]{media/co2-removal.png}
\end{center}

\paragraph{Problems with CCS}
\begin{itemize}
    \item CCS power plants are much more expensive than conventional coal-power plants
    \item CCS is still an unproven technology that might remove only parts (perhaps 25-35\%) of the CO$_2$ emissions
    \item CCS process is energy intensive $\to$\ cancels out some of the gains made
    \item CCS promotes continued use of coal
    \item No leaks are allowed $\to$\ Stored CO$_2$ needs to remain sealed forever
\end{itemize}

\section{Coal combustion alternatives}
\subsection{Coal gasification}
\begin{itemize}
    \item Solid coal can be converted into synthetic natural gas (SNG) - coal gasification
    \item It can be converted into a liquid fuel (e.g. methanol, synthetic gasoline) - coal liquefaction
    \item Classified as synfuels
    \item Production of gaseous and liquid fuels requires 50\% more coal for equal energy output\\
        $\to$\ 50\% more CO$_2$ to the atmosphere and more than 4 times higher water requirement
    \item Synfuels have low net energy yield and higher production cost per unit of energy
\end{itemize}

\subsection{Biomass}
Biomass can be burned directly as solid fuel or converted to liquid
biofuel (biodiesel, bioethanol and biogas).

\subsubsection{Combustion of biomass for electricity production}
\begin{itemize}
    \item Solid biomass can be used for combustion or co-firing in coal-fired power plants
    \item Biomass is normally used as chips (wood), pellets (unprocessed or torrefied) or bales (straw)
\end{itemize}

\subsection{Coal-fired power plants to biomass plants}
\begin{center}
    \includegraphics[width=.75\textwidth]{media/coalfiring.png}
\end{center}
\begin{enumerate}
    \item Coal is fired together with a modest ratio of biomass pellets
    \item Coal power plants that only use biomass as combustion fuel
    \item Mills are specially designed for biomass fuels
    \item Exchange of original coal burners with specially designed biomass burners
    \item Gasification of biomass
\end{enumerate}

\part{12}
\section{Environmental Economics}
\subsection{Definition of economics}
Economics is the study of production and consumption of goods and the
transfer of wealth to produce and obtain those goods.

Economics explains how people interact within markets to get what
they want.

Economics is differentiated in macro- and microeconomics:
\begin{itemize}
    \item Microeconomics focuses on the actions of individuals and industries (e.g. dynamics between buyers and sellers)
    \item Macroeconomics takes a broader view by analyzing the economic activity of for example an entire country
\end{itemize}

\subsection{Economic system / Direct costs}
System through which goods and services are produced, distributed and consumed.

Three types of capital (resources) are used within the economic system:
\subsubsection{Human capital}
Human capital refers to the knowledge, skills, experience, health,
and capabilities of individuals. It includes education, professional
training, creativity, problem-solving ability, and the capacity of
people to contribute effectively to economic and social activities.

\subsubsection{Industrial capital}
Industrial (or manufactured) capital includes human-made physical
assets used in production processes. Examples include machinery,
equipment, factories, buildings, infrastructure, and technological
systems that support the creation of goods and services.

\subsubsection{Natural capital}
Natural capital refers to the resources and ecosystem services
provided by nature. It includes renewable and non-renewable
resources such as water, soil, forests, minerals, air, biodiversity,
and the natural processes that support life and economic activity.

\subsection{Indirect costs}
Indirect costs are costs caused by production or consumption that are
not paid directly by the producer or consumer. They are often shifted
to society, future generations or the environment.

Examples of indirect costs:
\begin{itemize}
    \item Environmental damage, such as air pollution, water pollution, soil degradation and loss of biodiversity
    \item Health costs caused by pollution, toxic substances, noise or unsafe working conditions
    \item Climate change costs, such as damage from extreme weather events, sea-level rise and reduced agricultural productivity
    \item Resource depletion, for example overuse of fossil fuels, minerals, forests, groundwater or fish stocks
    \item Waste treatment and cleanup costs, such as contaminated land remediation or plastic pollution removal
    \item Social costs, such as reduced quality of life, displacement of communities or unequal distribution of environmental burdens
\end{itemize}

These costs are also called external costs or negative externalities,
because they are external to the market price. If indirect costs are
not included in the price of goods and services, the market price is
too low and consumption is higher than the socially optimal level.

\subsection{Free-market economic system}
\begin{itemize}
    \item No government inferences (e.g. subsidies, tax)
    \item Perfect information exchange between buyer and seller (no misleading)
    \item Prices of goods and services include all direct and indirect costs (e.g. environmental costs)
\end{itemize}

\subsection{Environmental economics}
\begin{itemize}
    \item Environmental economics is a distinct branch of economics
    \item It acknowledges the value of both the environment and economic activity and makes choices based on those values
    \item Goal is to balance economic activity and environmental impacts by taking into account all costs and benefits
    \item Theories are designed to take into account pollution and natural resource depletion $\to$\ classical economic models fail to do so
\end{itemize}

\subsubsection{Problems with environmental economics}
\paragraph{Scarcity}
\begin{itemize}
    \item Historically goods/services provided by the environment were seen to be limitless (not scarce)
    \item Scarcity is a result of misallocation of these services (which are not limitless)
    \item Reason is a pricing problem (for free or too low costs) $\to$\ if resources were properly priced, then they could not be over-exploited, since actual cost would be too high
\end{itemize}

\paragraph{Open-access resources}
\begin{itemize}
    \item Classical economic system cannot prevent degradation of open-access resources (e.g. clean air, oceans), since they are not bought and sold in the marketplace (market failure)
    \item Open-access resources are not owned by anyone and are available for everyone
    \item Opposite are private resources/goods
\end{itemize}

\subsubsection{Types of property / resource right}
\begin{itemize}
    \item Private property: A resource is owned by an individual or organization, which can exclude others and control its use.
    \item Common property: A resource is shared by a defined group, with rules or agreements controlling access and use.
    \item Open access resources: A resource is available to everyone without effective ownership or restrictions, making overuse likely.
\end{itemize}

\subsubsection{External costs (externality)}
\begin{itemize}
    \item Uncompensated costs of human actions, which needs to be addressed by correcting prices
    \item Negative externality $\to$\ Effect on ``environment'' is adverse
    \item Positive externality $\to$\ Effect on ``environment'' is beneficial
\end{itemize}

\subsubsection{Market failure (negative externality)}
\begin{itemize}
    \item Inability of markets to reflect the full social costs or benefits of a good or service
    \item Environment provides resources, assimilates waste and provides aesthetic pleasure
    \item Positive economic value is not recognized by classical economics $\to$\ Ability to set a price for these services is missing
    \item Environmental economics is a tool to overcome market failure with respect to resource use and pollution
\end{itemize}
\begin{center}
    \includegraphics[lw]{media/market-failure.png}
\end{center}

\subsection{Coarse Theorem}
\begin{itemize}
    \item Proposition that if private parties can bargain without cost over the allocation of resources, the problem of externalities is solved on its own
    \item Example: Mark owns a dog, which is barking constantly which disturbs Ellen (noise pollution):\\
        $\to$\ Solution A: Ellen pays Mark to get rid of the dog\\
        $\to$\ Solution B: Mark pays Ellen to let him keep the dog
    \item Problem: private solutions do not work most of the time:\\
        $\to$\ Assumption of free transaction cost (cost occur in the process of bargaining)\\
        $\to$\ Various interest groups might be involved (coordination problem)
\end{itemize}

When no private solutions can be reached, governments need to interfere:
\begin{itemize}
    \item Command and control policies: regulate behavior directly (e.g. laws, regulations, standards)
    \item Market-based policies: Provide incentives so that private entities will choose to solve the problem on their own
\end{itemize}

\subsection{Policy options}
\subsubsection{Economic efficiency}
\begin{itemize}
    \item Economic efficiency is defined as the maximization of social welfare: An efficient market is one that allows society to maximize the net present value (NPV) of benefits
    \item Aka Pareto optimal: state of resource allocation in which it is not possible to make any individual better off without making at least one individual worse off
\end{itemize}
 
\subsubsection{Cost-effectiveness}
\begin{itemize}
    \item A policy is cost-effective if it meets a given goal at least cost (minimizing abatement cost)
    \item Cost-effectiveness does not evaluate whether that goal has been set appropriately to maximize economic efficiency
\end{itemize}

\section{Strategies of environmental economics}
\subsection{Economic / Environmental indicators}
\subsubsection{Genuine progress indicator (GPI) / Gross domestic product (GDP)}
\paragraph{GDP}
\begin{itemize}
    \item Measures the annual economic value of all goods and services produced within a country
    \item GDP does not distinguish between goods and services that are environmentally beneficial or harmful
\end{itemize}

New indicators are required, which also monitor environmental quality and human well-being

\paragraph{GPI}
\begin{itemize}
    \item Estimated harmful environmental costs (e.g. pollution, resource depletion, degradation) and social costs (such as crime) are subtracted from GDP
\end{itemize}

\begin{center}
    \includegraphics[\lw=.45]{media/gpi.png}
\end{center}

\subsubsection{Happy planet index (HPI)}
Instead of using GDP, the success of a country is calculated by:
\[\text{HPI} = \frac{(\text{country's life satisfaction index}) \cdot (\text{average life expectancy})}{\text{ecological footprint}}\]

\subsubsection{Gross national happiness (GNH)}
GNH is a measure of a country’s efforts toward sustainable economic development
based on:\\
Conservation of natural environment, Preservation of cultural values,
Economic equity, Good governance.

\subsection{Full-cost pricing}
\begin{itemize}
    \item Full-cost pricing refers to prices including internal costs + external costs (externalities)
    \item Full-cost pricing would reduce resource waste, pollution and environmental degradation
    \item Improves human health by encouraging producers to invent more resource-efficient and less-polluting methods of production
    \item Would allow consumers to make more informed choices
    \item Jobs and profits would be lost in environmentally harmful businesses as consumers chose green products
    \item Jobs and profits would be created in environmentally beneficial businesses
\end{itemize}

\subsubsection{Implementation barriers}
\begin{itemize}
    \item Producers of harmful and wasteful products oppose such pricing
    \item Estimation of environmental and health costs have high uncertainties involved
    \item Most consumers do not connect harmful costs with certain goods and services: requires consumer education and eco-labeling
    \item Full-cost pricing requires strict government action
\end{itemize}

\subsection{Monetary value of natural capital}
Different ways exist to estimate monetary values of natural capital/pollution.

\subsubsection{Mitigation cost}
\begin{itemize}
    \item How much would it cost to move an endangered species to a new habitat?
    \item How much would it cost to clean up soil contaminated by acid rain?
\end{itemize}

\subsubsection{Willingness to pay}
\begin{itemize}
    \item How much would people pay to maintain a certain forest or to keep a certain species?
    \item How much would you pay to have an uncontaminated soil field?
\end{itemize}

\subsection{Discount rate}
\begin{itemize}
    \item Estimate of a resource’s future economic value compared to its present value\\
        $\to$\ Having something today may be worth more than it will be in the future
    \item Discount rate is a primary factor how resources (e.g. forests, rivers) are used
    \item 0\% discount rate means that a forest worth 1 million Euro today will still be worth 1 million Euro in 50 years
    \item Today many business and organizations (e.g. World Bank) use 10\% annual discount rate:
        \begin{itemize}[label=$\to$]
            \item Forest would be worth much less in 50 years
            \item Cuhng down forest as quick as possible makes the most economic sense
            \item Does not take into account economic value of ecological services provided by the forest
        \end{itemize}
    \item Process of choosing a discount rate is controversial
        \begin{itemize}[label=$\to$]
            \item High discount rates encourage rapid exploitation of resources for immediate payoffs
            \item 0\% or even negative discount rates would make use of non-renewable and renewable resources more sustainable
        \end{itemize}
\end{itemize}

\subsection{Cost-benefit analysis}
\begin{itemize}
    \item Tool for making economic decisions about pollution control and resources management
    \item Comparison of estimated costs and benefits for a certain action (e.g. implementing a pollution control regulation)
    \item Direct costs like land, labor, materials or pollution-control technologies are easy to estimate
    \item Indirect costs (e.g. clean air, biodiversity) are more difficult to estimate\\
        $\to$\ Depend on individual assumptions, value judgments and discount factors
    \item Cost-benefit projects potential outcomes
    \item Cost-benefit evaluates alternative actions
\end{itemize}

\subsection{Phase out of environmentally harmful subsidies / tax breaks}
\todo

\subsection{Establish laws and regulations to prevent pollution and resource degradation}
\subsubsection{Environmental regulations (prescriptive regulations)}
\subsubsection{Technology standard}
\subsubsection{Performance-based standard}

\subsection{Increase/establish taxes on pollution, resource waste and environmentally harmful goods and services}
\subsubsection{Market-oriented approaches (aka market-based approaches)}
\subsubsection{Emission taxes}
\subsubsection{Emission taxes (aka Green taxes or Pigovian taxes)}
\subsubsection{Green taxes}
\begin{itemize}
    \item Companies will only decrease pollution when the incentive is profitable
    \item Cost of reducing an additional unit of emissions $\to$\ Marginal abatement cost (MAC)
    \item MAC increases as the emission reduction amount increases
    \begin{itemize}[label=$\to$]
        \item Measures with lowest cost to reduce emissions are implemented first
        \item Additional reductions require more costly measures
    \end{itemize}
\end{itemize}
