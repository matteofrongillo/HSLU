...
Let $\dm \forall \theta \in \mathbb{R}$, then we can compute that:
\figbox{$\dm \sin^2(\theta) + \cos^2(\theta) = 1$}

\subsubsection{Example with $\dfrac{\pi}{4}$}
When $\dm \theta=\frac{\pi}{4}$, then $\dm \sin(\theta)=\cos(\theta) \Rightarrow 2\cos^2(\theta)=1
