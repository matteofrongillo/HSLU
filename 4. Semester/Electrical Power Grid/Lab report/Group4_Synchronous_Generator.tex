...
\begin{center}
    ...
    \textbf{\huge automatic generator control}\\[2.5cm]
...
may occur during connection. In this experiment, synchronisation was performed
manually using the dark lamp method and a synchronoscope. 

\subsection{Manual synchronisation with the aid of a dark lamp circuit}
For manual synchronisation of a synchronous generator to the power grid, the dark lamp
synchronisation method was used. The lamps were connected between identical phases of
the generator and the grid, as shown in \autoref{fig:darklamp}.
\begin{figure}[H]
    \centering
...
angle difference and the frequency difference between generator and grid, for
...
The generator was initially operated slightly above synchronous speed (+2rpm). The
...
synchronisation process. On the top right the dual-range voltmeter shows twice 400 V, the
zero voltmeter below shows 0 volts. On the left side the dual-range frequency meter
...
For a machine with non-salient pole or drum-type rotors operating at constant excitation
...
power. By utilizing a $\cos\Phi$ controller within the multifunction relay, the
...
Active and reactive power compensation experiment is not conducted; hence, only the
theoretical concept of their work is analyzed.

\subsection{Power regulation}
The conducted experiment involved semi-automatic changes. In such operating
mode, the automatics run up and synchronize the machine, and the operator specifies the
...
synchronous machine, test bench for servomachine, excitation voltage controller,
...
The voltage exciters were initially controlled when off. The multifunction relay was put
...
\begin{enumerate}
    ...
    viewer.
    \item Servo-machine test stand, three-phase power supply and excitation system were
    arranged as per lab guidelines.
    \item Multi-function relay was set to AUTO mode.
    \item SCADA diagnostics were initialized and synchronous machine was operated by
    STARTUP command.
    \item As soon as generator circuit breaker was closed, LOAD FOLLOWING was not turned
    ON.
    \item Limit of apparent power and active/reactive power references were imposed
    ...
    \item Experiment was ended using SCADA shutdown procedure.
\end{enumerate}
...
$P_\text{set} = 70\%$ [W] and $Q_\text{set} = 0\%$ [Var].
...
must press a key on a button to turn on the generator.
...
reactive power at the balance node and controls the motor-generator to provide a compensation
...
upwards/downwards the output of the motor-generator until the $\sum P$ and $\sum Q$
