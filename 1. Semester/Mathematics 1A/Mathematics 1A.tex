...
\subsubsection{Simplified definition}
Let $\forall \alpha \in \mathbb{R}$, then:
\figbox{$\dm f(x)=\alpha \Rightarrow f'(x)=\alpha \cdot x^{\, \alpha-1}$}
...
Without loss of generality, if we have a limit:
\[ \lim_{x \to x_0} \frac{f(x)}{g(x)} \]
where \(\dm \frac{f(x_0)}{g(x_0)}\) results in an indeterminate form, then:
