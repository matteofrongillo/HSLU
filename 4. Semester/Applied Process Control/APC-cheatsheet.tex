...
\begin{itemize}
    \item Signal processing: uses A/D and D/A converters, causing time/amplitude discretization and delays
    \item Dynamic performance: slightly reduced due to discretization
    \item Flexibility: parameters can be updated by software
    \item Cost and size: smaller and often cheaper hardware
    \item Multitasking: can perform additional tasks besides control
    \item Noise robustness: digital filtering improves noise resistance
    \item Complexity: supports advanced control algorithms
    \item Maintenance: easier diagnostics and software updates
    \item Scalability: easy integration, upgrades, and expansion
    \item Precision: high accuracy and stable performance over time
\end{itemize}

\subsection{Implementation platforms for digital controllers}
\begin{itemize}
    \item IPCs: industrial computers: complex real-time control tasks
    \item MCUs: compact, low-cost chips for simple embedded control
    \item PLCs: industrial controllers for automation in harsh env.
    \item DSPs: processors optimized for fast real-time signal calc.
    \item FPGAs: reconfigurable hardware for ultra-fast parallel control
    \item SBCs: compact computers: prototyping and low-cost control
    \item Cloud controllers: remote control, monitoring, and data analysis using cloud computing
\end{itemize}

\subsection{System architecture and communication}
\subsubsection{Centralized control}
\begin{itemize}
    \item One central unit handles signal processing, A/D and D/A conversion, inputs, and outputs
    \item Simple wiring, but possible bottlenecks in larger systems
\end{itemize}

\subsubsection{Fieldbus communication}
\begin{itemize}
    \item Sensors, actuators, and controllers exchange data over an industrial network
    \item Scalable, modular, and reduces wiring complexity
    \item (e.g. ProfiNET, CAN/CANopen, EtherCAT)
\end{itemize}
...
\includegraphics[\lw]{media-cheatsheet/PID2.png}
\[\dm u(t) = K_p\left(e(t) + \frac{1}{T}\integral[0][t][e(\tau)][\tau]+T_d\frac{de(t)}{dt}\right)\]

Controller gain: $K_p$\\
Integral time: $\dm T_i = \frac{K_p}{K_i}$\\
Derivative time: $\dm T_d = \frac{K_d}{K_p}$

\subsection{PID principle}
\pph{Proportional (P)}
Larger error $e(t)$ gives a larger control output $u(t)$

\pph{Integral (I)}
If $e(t)=0$, the control output keeps increasing or decreasing to
remove steady-state error

\pph{Derivative (D)}
Larger error change rate $\dot{e}(t)$ gives a stronger control action
to anticipate future error and reduce overshoot

\begin{center}
    \includegraphics[\lw=.8]{media-cheatsheet/PID-graph.png}
\end{center}
\begin{gather*}
    u(t_j) = K_p c_p + K_i c_i + K_d c_d\\
    u(t_j) = K_p \left(c_p + \frac{1}{T_i} c_i + T_d c_d\right)
\end{gather*}
