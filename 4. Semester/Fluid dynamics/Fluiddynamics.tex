...
\[p_1 - p_2 = \rho\frac{P_{\, \text{Diss}}}{\dot{m}} = \frac{P_{\, \text{Diss}}}{\dot{V}} = \frac{8\pi L \eta v_m}{R^2}\]
...
\begin{tabular}{clc|clc}
  ...
  \rowstrut $\delta$ & \makecell[l]{boundary layer\\thickness} & m & $\delta^*$ & \makecell[l]{displacement\\thickness} & m\\
  \rowstrut $\theta$ & \makecell[l]{momentum\\thickness} & m & $\tau_w$ & \makecell[l]{wall shear stress} & Pa\\
  \rowstrut $\nu$ & \makecell[l]{kinematic\\viscosity} & m$^2$/s & $Re_x$ & \makecell[l]{Reynolds number\\at position $x$} & --\\
  \rowstrut $x$ & \makecell[l]{distance from\\leading edge} & m & $\dot{V}_{BL}$ & \makecell[l]{boundary-layer\\volume flow rate} & m$^2$/s
\end{tabular}}
...
Transition to turbulent: $Re_x \geq 5e5$ (usually $3.2e5$ to $3e6$)
\vfill\phantom{}
...
\vspace*{-.2cm}
\begin{itemize}
  \item $\delta(x) \approx \sqrt{x}$
  \item $\tau(x) \approx 1/\delta(x) = 1/\sqrt{x}$
  \item $\dot{V}_{BL} = v_\infty \cdot \delta^*(x)$
\end{itemize}
...
\begin{center}
  \includegraphics[width=.75\linewidth]{media/turbolent_BL.png}
\end{center}
\begin{itemize}
...
\figbox{$\dm F_D = C_D\frac{1}{2}\rho v_\infty^2 A_p$}

where:
\begin{center}
  $C_D$: drag coefficient ($\mu$)$\quad ; \quad$$A_p$: project area
\end{center}
