...
\begin{wrapfigure}{r}{6cm}
    \begin{tikzpicture}[scale = 0.7, x=4cm, y=4cm, >=Stealth, declare function={
        func1(\x) = \x > -1 ? -0.3 * ln(\x + 1) : 0; % Define func1 only for x > -1
        func2(\x) = sin(5*\x r);
        x1 = -0.96365;
        x2 = -0.70284;
        x3 = 0;
        x4 = 0.6588;
    }, samples=200] % Increase samples for smoother curves
        % Fill areas between points of intersection
        ...
        \fill[cyan!30!white] plot[domain=x3:x4] (\x,{func1(\x)}) --
                                plot[domain=x4:x3] (\x,{func2(\x)}) -- cycle;
    
        % Draw func1
        \draw[blue, -, thick] plot[domain=-0.99:0.6588] (\x,{func1(\x)}); % Start at -0.99 to avoid ln(0)
        
        % Draw func2
        \draw[red, -, thick]  plot[domain=-1:0.6588] (\x,{func2(\x)});
    
        % Draw vertical dashed lines at intersection points
        \draw[-, dashed] (x1, 0) node[below] {\(a\)} -- (x1, {func1(x1)});
        \draw[-, dashed] (x2, 0) node[below] {\(b\)} -- (x2, {func1(x2)});
        \draw[-, dashed] (x3, 0) node[below right=1pt] {\(c\)} -- (x3, {func1(x3)});
        \draw[-, dashed] (x4, 0) node[above] {\(d\)} -- (x4, {func1(x4)});
...
Given a function \(y=f(x)\) and \(y=g(x)\), the area enclosed by the two functions
in the interval \(I=[a;d]\) is given by:

\begin{center}
    \fbox{$A = \integral[a][b][f(x)-g(x)][x] + \integral[b][c][g(x)-f(x)][x] + \integral[c][d][f(x)-g(x)][x]$}
\end{center}
\wrapfill

\vspace*{-3cm}
\subsection{Arc length}
Let a plane curve be given by $f(x)$ with $a\leq x \leq b$. Then the
arc length between $]a,f(a)[$ and $]b, f(b)[$ is:
