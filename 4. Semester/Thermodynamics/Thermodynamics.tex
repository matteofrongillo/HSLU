...
\begin{minipage}{.48\linewidth}
  \begin{align*}
    ...
    \left|q_{41}\right| &= h_4 - h_1 \text{ (condensor)}
  \end{align*}
\end{minipage}
...
\begin{gather*}
  \varepsilon_\text{HP} = \frac{\left|q_{23}\right|}{w_{12}} = \frac{h_2-h_3}{h_2-h_1} = \varepsilon_R + 1\\
  \varepsilon_R = \frac{\left|q_{41}\right|}{w_{12}} = \frac{h_1-h_4}{h_2-h_1} = \varepsilon_\text{HP} - 1
\end{gather*}

\subsection{Outer coefficient of performance (COP)}
\subsubsection{COP of a heat pump}
\includegraphics[\lw]{media/heat-pump.png}
\begin{gather*}
  \text{COP}_\text{HP} = \frac{\lvert\dot{Q}_\omega\rvert}{P_{el}} = \frac{\lvert\dot{Q}\rvert}{P_{el}}\\[1ex]
  \frac{1}{\eta_C} = \varepsilon_{HP,C} = \frac{T_C}{T_C - T_E} = \frac{T_C}{\Delta T_\text{lift}} = \varepsilon_R + 1\\[1ex]
  \varepsilon_{HP} > COP_{HP} \quad ; \quad \zeta_{HP} = \frac{COP_{HP}}{\varepsilon_{HP,C}}
\end{gather*}

\newcolumn
\subsubsection{COP of a refrigerator}
