...
many parallelotopes one over the other with side $a$ and volume $s^2 \cdot \Delta h$,
with $\Delta h \to 0$, in order to have infinitely may parallelotopes.

Computing the pyramid for n-slices:
\figbox{$\dm V =  \sum_{i=0}^{n-1} s_i^2 \cdot \Delta h = \integral[0][a][s^2][h]$}
...
\figbox{$\integral[0][h(a)][\left(h \cdot (h:s)\right)^2][h]$}
...
\begin{tikzpicture}[scale=1.5]
    ...
    \draw[->] (-0.5,0) -- (6,0) node[below right] {$x$};
    \draw[->] (0,-0.5) -- (0,3) node[above left] {$y$};
    
    \draw[domain=0:5, smooth, samples=100, thick, blue] plot (\x,{0.1*\x*\x - 0.4*\x + 1.5}) node[right] {$y = f(x)$};
    ...
    \fill[gray!30] (2.8,0) rectangle (3.8,{0.1*3.3*3.3 - 0.4*3.3 + 1.5});
    \draw[dashed] (2.8,0) -- (2.8,{0.1*2.8*2.8 - 0.4*2.8 + 1.5});
    \draw[dashed] (3.8,0) -- (3.8,{0.1*3.8*3.8 - 0.4*3.8 + 1.5});
    
    \draw[<->] (2.8,-0.2) -- (3.8,-0.2) node[midway, below] {$\Delta x$};
    
    \node[below] at (2.8,0) {$x_i$};
    \node[below] at (3.8,0) {$x_{i+1}$};
    
    \draw[->, thick] (5.5,0.2) arc[start angle=20, end angle=-220, radius=0.4];
    \node[right] at (5.8,-0.1) {};
\end{tikzpicture}
