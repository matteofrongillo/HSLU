...
\begin{itemize}
    ...
    \item $(a+b)(a^2-ab+b^2)=a^3+a^3$ (sum of cubes);
    \item $(a-b)(a^2+ab+b^3)=a^3-b^3$ (difference of cubes).
\end{itemize}
...
\figbox{$p(x)=a_n \cdot x^n + a_{n-1} \cdot x^{n-2} + a_{n-2} \cdot x^{n-1} + \dots + a_{2} \cdot x^{2} + a_{1} \cdot x^{1} + a_{0} \cdot x$}
...
The highest number has the highest degree element in a function, the highest is the influence of that element.
...
\figbox{$\ln(x) < x < x^2 < x^n < x^m < 2^x < e^x < 10^x < x^x$}
...
When the dominant element is at the nominator:
\figbox{$\lim_{x \to \infty}$ \Large $\frac{x^n}{x^{n-1}}$ \normalsize $= \infty$}

When the dominant element is at the denominator:
\figbox{$\lim_{x \to \infty}$ \Large $\frac{x^{n-1}}{x^n}$ \normalsize $= 0$}

When we have the same degree either in the nominator and in the denominator:
\figbox{$\lim_{x \to \infty}$ \Large $\frac{ax^n}{bx^n} = \frac{a}{b}$}

\defs{\textbf{h-asymptote} appears when $x$ approaches to $\infty$, which implieas that $y$
approaches to a number $A$ different from $\pm \infty$}
