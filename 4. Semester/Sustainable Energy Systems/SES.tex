...
\begin{center}
  \includegraphics[\lw=.8]{media/pipe-size.png}
\end{center}
...
\subsection{Economic feasibility indicators}
\[
  q_L = \frac{Q_\text{year}}{L}
  \qquad [\text{MWh}/(\text{km}\,a)]
\]
\begin{itemize}
  \item Linear heat density: annual heat demand per pipe length
  \item Rough profitability criterion: $q_L > 2000$ MWh/(km a)
...
\begin{itemize}
  \item Load duration curve: load sorted by frequency/duration over the year
...
\[
  f_\text{div} =
  \frac{\sum_i \dot Q_i(t_\text{max})}{\sum_i \dot Q_{\text{sub},i}}
\]
\begin{itemize}
  \item Consumers do not peak simultaneously; network power lower than sum of subscribed powers
  \item Small groups: orientation $\sim 0.85$; large groups: orientation $\sim 0.75$
...
\[
  \dot Q = \dot m c_p (T_\text{supply} - T_\text{return})
  = \dot m c_p \Delta T
\]
\[
...
\]
\begin{itemize}
  \item Higher $\dot Q$ needs higher $\dot m$, larger $\Delta T$, or larger pipe diameter
  \item For water: $c_p \approx 4.18$ kJ/(kg K)
  \item Example from handout: DN100 transport power scales with $\Delta T$: 20 K \to 1.3 MW, 40 K \to 2.6 MW, 60 K \to 3.9 MW
\end{itemize}

\subsection{Required network power}
\[
  \dot Q_\text{req} = \dot Q_\text{sub} f_\text{div} + \dot Q_l
\]
\begin{itemize}
  \item $\dot Q_\text{sub}$: subscribed/connected customer power
  \item $\dot Q_l$: network heat losses; classic DH order $\sim 2\%$ of required heat
  \item Nominal network power must include diversity and losses
\end{itemize}

\subsection{Peak load and reserves}
\[
  P_P = P_\text{sub}\,Sim + P_L - P_S
\]
\begin{itemize}
  \item $P_P$: peak load coverage; $P_S$: base source power; $P_L$: losses
  \item Reserve criterion: maintain supply if largest generator fails ($n-1$)
...
\begin{itemize}
  ...
  \item Scaling: $\Delta p_2 = \Delta p_1\left(\frac{v_2}{v_1}\right)^2$
  \item Design recommendation from handout: pressure loss $<250$ Pa/m
  \item Larger pipe: lower pumping energy but higher investment; optimum balances both
\end{itemize}
...
\[
  \dot Q =
  \frac{2\pi L\Delta T}{(1/\lambda)\ln(d_a/d_i)}
\]
\begin{itemize}
...
\[
  \Delta T = \frac{\dot Q_l}{\dot m c_p}
  \qquad \text{or} \qquad
  \Delta T = \frac{q_l L}{\dot m c_p}
\]
\begin{itemize}
...
\section{Stations and operation}
\subsection{Heat transfer station}
\begin{itemize}
  \item Transfers heat from primary network to secondary building circuit
  \item Measures heat by volume flow and supply/return temperatures
  \item Reduces pressure, limits volume flow/subscribed power, controls and records data
  \item Indirect stations with heat exchangers are common; direct stations are rare
\end{itemize}

\subsection{DHW and hot water}
\begin{itemize}
  \item DHW storage exit should reach 60\cel to avoid legionella
  \item DHW system choice affects secondary and primary return temperature
  \item Hot water $>110\cel$ means vapor pressure $>0.5$ bar rel.; PED/pressure rules become relevant
  \item Heat carriers: water, brine, steam, thermal oil; choose by temperature range, safety, cost, durability
\end{itemize}
